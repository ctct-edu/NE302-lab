...
`start/` フォルダには、Session 1 の完成物が含まれています：
...
export type ProductCategory = 'FOOD' | 'DRINK' | 'STATIONERY' | 'OTHER';
...
- **Union 型**（`'FOOD' | 'DRINK'`）で取りうる値を限定できる
...
・Order（id, items, status, total_amount, created_at, updated_at）

【関連】
・Product : Inventory = 1 : 1
・Product : Transaction = 1 : *
・Product : Order = 1 : *（items を通じて）
```
