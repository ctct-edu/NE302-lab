  ...
  describe('updateStock', () => {
    ...
    describe('異常系', () => {
      it('存在しない商品IDの場合、エラーを投げる', async () => {
        ...
        await expect(service.updateStock(999, 10, TransactionType.IN)).rejects.toThrow(
          '商品が見つかりません'
        );
      });
      ...
      it('在庫不足での出庫はエラーを投げる', async () => {
        ...
        await expect(service.updateStock(1, 51, TransactionType.OUT)).rejects.toThrow(
          '在庫が不足しています'
        );
      });
      ...
      it('数量が0の場合、エラーを投げる', async () => {
        ...
        await expect(service.updateStock(1, 0, TransactionType.IN)).rejects.toThrow(
          '数量は1以上で指定してください'
        );
      });
      ...
      it('数量が負の値の場合、エラーを投げる', async () => {
        ...
        await expect(service.updateStock(1, -5, TransactionType.IN)).rejects.toThrow(
          '数量は1以上で指定してください'
        );
      });
    });
    ...
    describe('境界値', () => {
      ...
      it('出庫後の在庫が-1になるケース（エラー）', async () => {
        ...
        // 100 - 101 = -1 はエラー
        await expect(service.updateStock(1, 101, TransactionType.OUT)).rejects.toThrow(
          '在庫が不足しています'
        );
      });
    });
